...
std::string roomMenu(std::unordered_map<std::string, std::string>& inv, X theRoom, int newline){
     ...
     std::cout << "\nPlease enter a command from this menu of options: \"Go to (room name)\" or just \"(room name)\", \"Inspect (object name)\", \"Talk to (person name)\", \"Interact with (animal name)\", \"Get in (object name)\", \"Sleep on (furniture name)\", \"Switch on (object name)\", \"Open (object/door name)\", or \"Play (instrument name)\". You may additionally check your inventory (\"Check inventory\").\n\n" << std::endl;
 //not using these: \"Give (object name)\", \"Use (object name)\", \"Add (object name) to inventory\", 
     //std::cin.clear();
     //std::cin >> choice;
     //std::cout << newline << std::endl;
     if(newline == false) //first time roomMenu function is called
     ...
     */
/*
     //check if choice matches item to add to inventory, not using this method
...
*/
     ...
     //check if choice matches hot tub
          ...
          {
               if (vstrings[2].compare("hottub") == 0)
                    choice = "hottub";
          }
     }
	  ...
	  newline = false;
          case 18:
               ...
               do{
                    ...
                    //logic to check against parsed input
                    if (roomReturn == "mr.deeds"){
                         choice = 1;
                    }
                    else if (roomReturn == "foldingtable"){
                         choice = 2;
                    }
                    else if (roomReturn == "attic"){
                         choice = 3;
	  ...
	  newline = false;
          case 20:
               ...
               do{
                    ...
                    //logic to check against parsed input
                    if (roomReturn == "mr.glass"){
                         choice = 1;
                    }
                    else if (roomReturn == "nightstand"){
                         choice = 2;
                    }
                    else if (roomReturn == "hallway"){
                         choice = 3;
