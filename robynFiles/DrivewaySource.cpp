...
int movePlayerFromDriveway(std::string location)
{
	int intChoice;
	if (location.compare("Mansion") == 0) {
...
int drivewayNavigate(std::unordered_map<std::string, std::string>& inventory){
	...
	//Game Intro
	std::cout << "\n\nThe Driveway\n\n\n\n" <<
		"Sherlock is in the driveway.\n\nThere is a fancy luxury car that you may want to take a look at (Enter \"Look at car\").\n\n" <<
		"There is also a man standing by the front of the driveway smoking a cigarette (To talk with him: Enter \"Talk to man\").\n\n" <<
		"You may also enter the mansion (Enter \"Mansion\")\n\nYou may go around the back to the garden (Enter \"Garden\").\n\n" <<
	...
	do{
		...
		do
		{
			...
			while (!std::cin)
			{
				std::cin.clear();
				std::cin.ignore(255, '\n');
				std::cout << "\n\nPlease enter a valid option (Look at car, Talk to man, Mansion, Garden or Check inventory) : \n\n";
				std::cin >> choice;
			}
			retry++;
			firstTime++;

		} while ((choice.compare("Look at car") != 0) && (choice.compare("Talk to man") != 0) &&
			(choice.compare("Mansion") != 0) && (choice.compare("Garden") != 0) && (choice.compare("Check inventory") != 0));


		//take users choice and interact based on that
		if (choice.compare("Look at car") == 0) {
		...
		else if (choice.compare("Talk to man") == 0) {
			mrWhite.conversate(inventory);
		}
		else if (choice.compare("Mansion") == 0 || choice.compare("Garden") == 0) {
			intChoice = movePlayerFromDriveway(choice);
			move = true;
		}
		else {
			std::cout << "\n\nInventory contains: \n\n";
