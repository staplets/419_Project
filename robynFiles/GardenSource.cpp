...
int movePlayerFromGarden(std::string location)
{
	int intChoice;

	if (location.compare("Driveway") == 0) {
...
int gardenNavigate(std::unordered_map<std::string, std::string>& inventory){
	...
	std::cout << "\n\nThe Garden\n\n\n\n" <<
		...
		"There is woman dressed in a black evening gown smoking a cigarette on a bench, who you may want to talk with (Interact: Enter \"Talk to woman\").\n\n" <<
		"There is also a dog laying down chained to a water fountain in front of what looks like a Hydrangea plant (Interact: Enter \"Interact with dog\").\n\n" <<
		"You may also go around to the driveway (Enter \"Driveway\")\n\nYou may go up on the deck (Enter \"Driveway\").\n\n" <<
		"You may go down into the cellar ( Enter \"Cellar\").\n\n" <<
	...
	do{
		...
		do
		{
			if (retry > 0)
			{
				...
				if (firstTime != 0){
					...
					std::cout << "\n\nThe Garden\n\n\n\n" <<
						...
						"There is woman dressed in a black evening gown smoking a cigarette on a bench, who you may want to talk with (Interact: Enter \"Talk to woman\").\n\n" <<
						"There is also a dog laying down chained to a water fountain in front of what looks like a Hydrangea plant (Interact: Enter \"Interact with dog\").\n\n" <<
						"You may also go around to the driveway (Enter \"Driveway\")\n\nYou may go up on the deck (Enter \"Deck\").\n\n" <<
						"You may go down into the cellar ( Enter \"Cellar\").\n\n" <<
						"Check your inventory of items ( Enter \"Check inventory\").\n\n" <<
						"What would you like to do?";
				}
			...
			while (!std::cin)
			{
				std::cin.clear();
				std::cin.ignore(255, '\n');
				std::cout << "\n\nPlease enter a valid option (Talk to woman, Interact with dog, Driveway, Deck, Cellar or Check inventory) : \n\n";
				std::cin >> choice;
			}
			retry++;
			firstTime++;

		} while ((choice.compare("Talk to woman") != 0) && (choice.compare("Interact with dog") != 0) && (choice.compare("Driveway") != 0)
			&& (choice.compare("Deck") != 0) && (choice.compare("Cellar") != 0) && (choice.compare("Check inventory") != 0));


		//take users choice and interact based on that
		if (choice.compare("Driveway") == 0 || choice.compare("Deck") == 0 || choice.compare("Cellar") == 0) {
			intChoice = movePlayerFromGarden(choice);
			move = true;
		}
			...
			if (choice.compare("Talk to woman") == 0) { //if player wants to talk to Ms. Wolf
