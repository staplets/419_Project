...
int movePlayerFromSitting(std::string location)
{
	int intChoice;
	if (location.compare("Smoking room") == 0) {
		std::cout << "\nWalking into the Smoking Room.\n";
		//change choice to reflect our room mapping and update move
		intChoice = 6;
	}
	else if (location.compare("Foyer") == 0) {
...
int sittingNavigate(std::unordered_map<std::string, std::string>& inventory){
	...
	std::cout << "\n\nThe Sitting Room\n\n\n\n" <<
		...
		"There is woman dressed in a peach dress reading through a magazine, who you may want to talk with (Interact: Enter \"Talk to woman\").\n\n" <<
		"You can also examine the couch (Interact: Enter \"Examine couch\").\n\n" <<
		"You may go around into the Smoking Room (Enter \"Smoking room\")\n\nYou may go out to the Foyer (Enter \"Foyer\").\n\n" <<
	...
	do{
		...
		do
		{
			...
			while (!std::cin)
			{
				std::cin.clear();
				std::cin.ignore(255, '\n');
				std::cout << "\n\nPlease enter a valid option (Talk to woman, Examine couch, Smoking room, Foyer, or Ballroom or Check inventory) : \n\n";
				std::cin >> choice;
			}
			retry++;
			firstTime++;

		} while ((choice.compare("Talk to woman") != 0) && (choice.compare("Examine couch") != 0) && (choice.compare("Smoking room") != 0)
			&& (choice.compare("Foyer") != 0) && (choice.compare("Ballroom") != 0) && (choice.compare("Check inventory") != 0));


		//take users choice and interact based on that
		if (choice.compare("Talk to woman") == 0) {
			mrsDeeds.conversate(inventory);
		}
		else if (choice.compare("Examine couch") == 0) {
			couch.checkCouch(inventory);
		}
		else if (choice.compare("Smoking room") == 0 || choice.compare("Foyer") == 0 || choice.compare("Ballroom") == 0) {
			intChoice = movePlayerFromSitting(choice);
			move = true;
		}
