...
void foyerDescription(std::unordered_map<std::string, std::string>& inv){
     ...
     std::cout << "\n\n" << fRoom.getName() << "\n\n\n\n" << fRoom.getDescription();

     std::cout << "There are also " << fRoom.getNumRooms() << " exits from this room: \n\n";
     
     for (i = 0; i < fRoom.getNumRooms(); i++){
          //output the room exits
          std::cout << i + 1 << "." << " " << fRoom.getExits()[i] << "\n";
...
int foyerNavigate(std::unordered_map<std::string, std::string>& inventory){
     ...
     //give description of foyer
     foyerDescription(inventory);
