...
void inspectCellarObj(std::string objName, std::unordered_map<std::string, std::string>& inventory)
{
	...
	if (objName.compare("Check dresser") == 0) { //if object is dresser
												 //formatting
		std::cout << "\n\n////////////////////////////////////////////////////////////////////////////////////\n\n";
		...
		box.checkBox(inventory); //if object is box
	}
}

int movePlayerFromCellar(std::string location)
{
	int intChoice;
	if (location.compare("Garden") == 0) {
...
int cellarNavigate(std::unordered_map<std::string, std::string>& inventory) {
	...
	//Game Intro
	std::cout << "\n\nThe Cellar\n\n\n\n" <<
		"Sherlock is in the Cellar.\n\nLooking around, he sees a Dresser in the corner and a Box in the other far corner that may be worth a look.\n\nLook at Dresser? (Enter \"Check dresser\").\n\n" <<
		"Open the Box? (Enter \"Open box\").\n\n" <<
		"You may go to the Garden outside (Enter \"Garden\")\n\nYou may go up to the Kitchen (Enter \"Kitchen\").\n\n" <<
